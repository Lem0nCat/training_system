...
def tests_list(request):
    if not request.user.is_authenticated:
        messages.error(request, 'Для выполнения этого действия войдите в аккаунт или зарегистрируйтесь!')
        return redirect('..')
...
def question_list(request, lecture_id):
    ...
    if request.user.is_teacher() == False and TestResult.objects.filter(user=request.user, test=test).exists():
    ...
    bool_test = Test.objects.filter(lecture_id=lecture_id).exists()
    context = {
        'test': bool_test,
        'questions': questions,
